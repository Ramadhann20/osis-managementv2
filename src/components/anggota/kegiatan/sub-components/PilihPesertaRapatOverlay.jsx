...
export default function PilihPesertaRapatOverlay({
  mode = "kelompok",
  member,
  divisi,
  ...
  const data = useMemo(() => {
    ...
    const preset = [
      ...
      {
        key: "sekbid_saya",
        label: `Sekbid Saya · ${labelDivisi(divisi)}`,
        description: "Seluruh anggota aktif pada divisi atau sekbid pengaju.",
        icon: "groups",
        members: sekbidSaya,
      },
      {
  ...
  return (
    <div className="fixed inset-0 z-[140] flex items-center justify-center p-3 sm:p-5">
      <style jsx global>{`
        @keyframes pesertaRapatBackdropMasuk {
          from { opacity: 0; }
          to { opacity: 1; }
        }
        @keyframes pesertaRapatPanelMasuk {
      ...
      <button
        ...
        onClick={onClose}
        style={{ animation: "pesertaRapatBackdropMasuk 160ms ease-out both" }}
        className="absolute inset-0 bg-on-surface/55 backdrop-blur-sm"
      />

      <section
        style={{
          animation: "pesertaRapatPanelMasuk 220ms cubic-bezier(0.22, 1, 0.36, 1) both",
        }}
        ...
        <header className="flex items-start justify-between gap-4 border-b border-border bg-card p-5 sm:p-6">
          <div>
            ...
            <p className="mt-2 max-w-xl text-sm leading-6 text-text-muted">
              {mode === "manual"
                ? "Cari anggota aktif lalu tambahkan beberapa orang ke daftar peserta rapat."
