...
export default function KegiatanDetailsModal({ activity, onClose }) {
  ...
  /**
   * Hapus permanen hanya tersedia pada detail sisi Pembina.
   *
   * Helper tidak hanya menghapus dokumen Kegiatan, tetapi juga membersihkan
   * Proposal, PelaksanaanKegiatan, SesiAbsensi, Absensi terkait dan menghitung
   * ulang RingkasanAbsensi anggota yang terdampak.
   */
  const handleDeleteActivity = async () => {
    ...
    try {
      const result = await hapusKegiatanBersih({
        ...
        activity: {
          ...activity,
          // State proposal lokal dipakai juga agar proposal yang baru masuk
          // tetap dapat ditemukan walaupun parent belum sempat rerender.
      ...
      // useCollection pada Manajemen Kegiatan bersifat realtime. Setelah root
      // Kegiatan hilang, card otomatis ikut hilang tanpa refresh manual.
      window.setTimeout(() => onClose?.(), 220);
    } catch (deleteError) {
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="mt-1 max-w-2xl text-xs leading-5 text-red-700">
                    ...
                    yang terhubung ke kegiatan ini akan ikut dibersihkan.
